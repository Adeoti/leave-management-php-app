				...
				<div class="col-sm-10 col-md-10">
						...
						<?php
							if(isset($_POST['add_dept'])){
								$name = htmlspecialchars(trim($_POST['iname']));
									$create = new Admin();
										$create -> addDeptLevel($name,"department");
							}
					...
					<hr/>
						...
						<?php
							...
							$fetch_deparment = $con -> prepare("SELECT * FROM department ORDER BY name");
										...
										while($row = $fetch_deparment -> fetch()){
												...
												$date_dept = $row['date'];
													echo '<tr>
														...
														<td>'.$sn.'</td>
														<td contenteditable class="editable" data-ref="'.$ref_dept.'">'.$name_dept.'</td>
														<td>'.$date_dept.'</td>
														<td>
															<a href="javascript:void(0)" class="btn editbull btn-sm btn-info"><i class="fa fa-pencil"></i> edit</a>
															<a href="trash.php?ref='.$ref_dept.'&tab=department&path=department" class="btn btn-sm btn-danger"><i class="fa fa-trash"></i> trash</a>
														</td>
													</tr>';
										}
										echo '</table>';
	...
	<script>
		$(document).ready(function(){
			...
			$('.editbull').click(function(){
				$(this).parent('td').siblings('td.editable').focus();
			...
			//alert(oldCrew);
				...
				let theRef = $(this).data('ref');
					$.get('savechanges.php',{agent:theRef, newinput:newVal, whereTo:'department', oldVal:oldCrew},function(data){
						if(data != ""){
							window.alert(data);
						}
					});
