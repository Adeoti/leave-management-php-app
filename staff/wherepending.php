<?php
	...
	$fetch = "SELECT * FROM leavee WHERE staff_dept = ? AND staff_ref != ? AND leave_status = ?";
								$fetch_leaveHistory = $con -> prepare($fetch);
									if($fetch_leaveHistory -> execute(array($_SESSION['sid_dept'],$_SESSION['sid_ref'],0))){
											...
											$sn = 0;
												...
												while($row = $fetch_leaveHistory -> fetch()){
													...
													$date_requested = $row['date_requested'];
														$ctrl = "
															<a href='javascript:void(null)' data-ref = '$ref' data-staff='$staff_ref' class='btn decline btn-sm btn-danger' title='click to decline'><i class='fa fa-times'></i></a>
															<a href='javascript:void(null)' data-ref = '$ref' data-staff='$staff_ref' class='btn forward btn-sm btn-primary' title='click to forward'><i class='fa fa-check'></i></a>
